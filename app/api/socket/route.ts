  ...
  for (const [sessionId, session] of userSessions.entries()) {
    if (now - session.lastSeen > fiveMinutes) {
      ...
      if (session.roomId && rooms.has(session.roomId)) {
        ...
        room.users = room.users.filter((user:any) => user.id !== session.userId);
    ...
    switch (action) {
      case "join-room":
        ...
        return Response.json({
          success: true,
          sessionId: newSessionId,
          roomUsers: room.users.map((u:any) => u.id),
        });
      ...
      case "get-room-users":
        ...
        return Response.json({ users: roomData.users.map((u:any) => u.id) });
  ...
  try {
    ...
    switch (action) {
      case "toggle-audio":
        ...
        if (rooms.has(roomId)) {
          const room = rooms.get(roomId);
          const otherUsers = room.users.filter((user:any) => user.id !== userId);
          ...
          return Response.json({
            ...
            targetUserId: userId,
            affectedUsers: otherUsers.map((u:any) => u.id),
          });
        }
